...
from app.auth.forms import (LoginForm, ManageUserForm, RegistrationForm,
...
def login():
    ...
    if current_user.is_authenticated:
        return redirect(url_for('swipes.home'))
    form = LoginForm()
...
def manage():
    ...
    if not current_user.admin:
        ...
        return redirect(url_for('swipes.home'))
    users = (User.query.order_by(User.password_hold.desc()).order_by(
        User.retired).order_by(User.admin.desc()).order_by(
            func.lower(User.firstname)).all())
    for user in users:
    ...
    for form in manageforms:
        if form.submit_.data and form.validate_on_submit():
            if (form.action_.data == 'ret') or (form.action2_.data == 'ret'):
